...
class ImageProcessor:
    ...
    def correct_distortion(self, raw_image):
        ...
        self.image_corrected_cropped = self.image_corrected[y_corrected[0]: y_corrected[-1], :]
    ...
    def show_image_with_edges(self, plot_image = False):
        ...
        self.image_with_edges[self.edge_1_fit - self.padding, rows] = np.max(self.image_with_edges)
        self.image_with_edges[self.edge_2_fit + self.padding, rows] = np.max(self.image_with_edges)
...
def main():
    ...
    xmin, xmax = 450, 690
    ymin, ymax = 115, 225
    img_corrected, img_corrected_cropped = processor.correct_distortion(raw_image=bias_image)

...
def extract_edges(image, method='sobel', threshold=0.1):
    """
...
def find_sensor_edges(image, threshold=0.8, show_dim = True):
    """
...
def func_distortion_correction(image_raw, upper_edge_line, lower_edge_line, padding = 0):
...
# Example usage:
upper_edge, lower_edge, upper_edge_fit, lower_edge_fit = find_sensor_edges(parallel_image, threshold=0.8)
print(upper_edge[0:10])
print(lower_edge[0:10])
upper_bound = np.round(np.mean(upper_edge)-50).astype(int)
lower_bound = np.round(np.mean(lower_edge)+50).astype(int)
# print((f"{upper_bound = }"))
# print((f"{lower_bound = }"))
padding = 10
image_corrected = func_distortion_correction(bias_image, upper_edge_fit, lower_edge_fit, padding = padding)
...
# plt.imshow(np.ma.masked_where(edges == False, edges), cmap='hsv')
# # plt.colorbar(img)
